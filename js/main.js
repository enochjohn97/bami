(function() {
    ...
    if (palmField) {
        var rows = [
            ...
            { y: 275, count: 7, scale: 0.10, colorA: "#0E3A2B", colorB: "#134B37" }
        ];
        var svgHtml = '';
        rows.forEach(function(row, ri) {
            for (var i = 0; i < row.count; i++) {
                var x = 60 + i * (400 / row.count) + (ri % 2 === 0 ? 10 : 0);
                svgHtml += '<g transform="translate(' + x + ',' + row.y + ') scale(' + row.scale + ')">' +
                    '<rect x="-6" y="-140" width="12" height="140" rx="6" fill="#0E3A2B"/>' +
                    '<path d="M0 -140 C -70 -160 -110 -120 -130 -80 C -80 -100 -40 -110 0 -140" fill="' + row
                    .colorA + '"/>' +
                    '<path d="M0 -140 C 70 -160 110 -120 130 -80 C 80 -100 40 -110 0 -140" fill="' + row
                    .colorA + '"/>' +
                    '<path d="M0 -140 C -50 -180 -20 -210 0 -230 C 20 -210 50 -180 0 -140" fill="' + row
                    .colorB + '"/>' +
                    '</g>';
            }
        });
    ...
    var stages = journeyTrack ? journeyTrack.querySelectorAll('.journey-stage') : [];
    ...
    function setStage(idx) {
        ...
        stages.forEach(function(s) {
            s.classList.toggle('active', parseInt(s.getAttribute('data-stage'), 10) === idx);
        });
    }
    ...
    function updateCalculator() {
        ...
        var extraction = parseFloat(document.getElementById('extractionRate').value);
        var pricePerGallon = parseInt(document.getElementById('pricePerGallon').value, 10);
        var costPerTree = parseInt(document.getElementById('costPerTree').value, 10);
        ...
        document.getElementById('pricePerGallonVal').textContent = formatNaira(pricePerGallon);
        document.getElementById('costPerTreeVal').textContent = formatNaira(costPerTree);

        // Calculations
        var totalFFBtons = trees * bunches * bunchKg / 1000;
        var cpoLitres = totalFFBtons * extraction * 10;  // because 20% extraction = 200 L/ton FFB
        ...
        document.getElementById('cpoLitres').textContent = Math.round(cpoLitres).toLocaleString();
        document.getElementById('gallons20L').textContent = Math.round(gallons20L).toLocaleString();
        document.getElementById('revenue').textContent = formatNaira(Math.round(revenue));
        document.getElementById('annualCost').textContent = formatNaira(Math.round(annualCost));
        document.getElementById('netProfit').textContent = formatNaira(Math.round(netProfit));
    }
